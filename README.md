...
받침대에는 **양옆 벽이 없다.** 물건이 받침대를 벗어나면 목숨이 하나 줄고, 목숨 3개가 다 떨어지면 끝난다. 단어를 놓치면 그 단어가 아레나 위에서 4.5초 기다리는 **예고 물건**이 되고, 그 사이 아무 단어나 맞히면 하나를 상쇄한다 — 뿌요뿌요의 예고 가비지와 같은 구조다. 난이도는 12초마다 한 단계씩 5단계까지 오르고, 상단에 단계와 다음 단계까지의 게이지가 보인다. 콤보는 단어를 맞출 때마다 오르고 목숨을 잃을 때만 초기화된다. 타자게임이므로 분당 타수도 함께 보여준다 — 맞춘 단어의 두벌식 키 수를 경과 시간으로 나눈 값이다.
...
"놓친 단어에 대가를 붙인다"는 제안을 훨씬 좋은 형태로 되돌려준 요청이다. 즉시 낙하는 대응할 방법이 없고, 예고 후 상쇄는 만회 여지가 남는다. 상쇄 조건은 "아무 단어나 하나"로 골랐다 — 놓친 그 단어를 다시 치게 하면 새로 내려오는 단어와 손이 경합해 리듬이 끊긴다.
