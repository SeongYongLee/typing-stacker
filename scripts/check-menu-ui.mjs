...
try {
  for(const mobile of [false,true]) {
    ...
    const signature=async button=>button.evaluate(e=>{const s=getComputedStyle(e);return [s.fontSize,s.fontWeight]})
    const options=page.getByRole('button',{name:'옵션',exact:true})
    const standard=await signature(options)
    assert((await options.boundingBox()).height>=44, 'Menu touch target is too small')
    ...
    await page.getByRole('button',{name:/돌아가기/}).click()
    if(mobile && process.env.BROWSER_SCREENSHOT){await page.screenshot({path:process.env.BROWSER_SCREENSHOT})}
