...
await page.goto('http://127.0.0.1:5183/');await page.waitForSelector('[data-room-light="3d"][data-ready="true"]');assert.ok((await page.locator('.title-splash__background').getAttribute('src')).endsWith(`arena/background-${night?'night':'day'}.webp`));await page.waitForTimeout(900);await page.screenshot({path:`/tmp/title-room-${mobile?'mobile':'desktop'}-${night?'night':'day'}.png`});assert.equal(await page.locator('[data-room-light]').evaluate(c=>getComputedStyle(c).pointerEvents),'none');
