...
import { StageStoryScene } from '../screens/StageStoryScene.tsx'
import { STAGE_STORIES } from '../screens/stageStories.ts'
...
export function DoorEntrance({ theme, onFinish }: { theme: TitleTheme; onFinish: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(onFinish, window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 2100)
  ...
  return <div className="door-entrance" data-theme={theme} onKeyDown={event => {
    ...
    <div className="door-entrance__room" aria-hidden="true"><StageStoryScene story={STAGE_STORIES[1]!} /></div>
