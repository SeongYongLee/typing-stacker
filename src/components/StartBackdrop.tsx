 ...
 * ## 밤으로 깐다
 *
 * 판은 첫 밤에서 열린다(`DayNight.ts`). 낮 그림을 깔아두면 시작하는 순간 방이
 * 어두워져서, 이어지는 대신 한 번 튄다. 값을 적어두지 않고 판이 열리는 시각
 * (`timeOfDay(0)`)을 그대로 넘기는 이유가 이것이다 — 국면 길이를 손봐도 저절로
 * 따라오고, 벽시계 바늘도 판이 시작할 자리에 미리 가 있는다.
 */
