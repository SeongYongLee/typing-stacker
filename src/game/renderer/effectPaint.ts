 ...
 */
function drawLandingGlow(view: ArenaView, landing: LandingGlow): void {
...
function drawTrails(
  ...
  const { ctx } = view
  ctx.save()
  for (const particle of trails.particles) {
    ...
    traceTrail(ctx, particle.kind, {
      x: view.toScreenX(particle.x),
      y: view.toScreenY(particle.y),
      /*
       * 김은 피어오르며 퍼진다(`grow`). 크기가 그대로면 위로 흐르는 점들이라
       * 연기가 아니라 거품으로 보인다.
       */
      radius: Math.max(0.9, particle.size * grownBy(particle) * view.scale),
      /*
...
function drawFormingLedge(
   ...
   */
  const toY = view.toScreenY(forming.y - LEDGE.halfHeight)
  const cx = fromX + (toX - fromX) * ease
  const cy = fromY + (toY - fromY) * ease

  const width = forming.halfWidth * 2 * view.scale
  ...
  const dust = sprite(artUrl(view.nightfall > 0.5 ? 'ledge-night' : 'ledge-day'))
  if (dust !== null) {
