...
export function MobileGame({ engine, store, onHome, onRestart, touch = true, arena, windowLight = true, showMergeToast = arena == null }: {
  showMergeToast?: boolean; arena?: ReactNode; windowLight?: boolean; touch?: boolean; engine: GameEngine; store: EngineStateStore; onHome?: () => void; onRestart?: () => void
}) {
  ...
  useLayoutEffect(() => {
    if (arena != null || canvas.current === null) return
  ...
  }, [engine, onHome, state?.runSeq, arena])
  useEffect(() => {
    const hide = () => {
      if (document.hidden) {
  ...
  return <div className="mp-game" data-game-screen data-paused={state.phase === 'paused'} data-controls={touch ? 'mobile' : 'pc'} data-phase={state.phase} data-awaiting-start={awaitingStart}>
    ...
    <div className="mp-arena">
      {arena ?? <canvas ref={canvas} aria-label="상자와 쌓인 물건" />}
