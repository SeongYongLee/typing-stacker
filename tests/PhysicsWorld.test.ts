...
function variantOf(word: string, hidden = false): ItemVariant {
  ...
  if (found === undefined) throw new Error(`no variant: ${word}`)
  return found
}
...
describe('PhysicsWorld', () => {
  ...
  it('받침대 가운데로 떨어진 물건은 안정적으로 멈춘다', () => {
    ...
    const expectedTop = ARENA.platformTop + halfExtentY(brick.shape) * 2
  ...
  it('스냅샷은 렌더러가 필요한 값을 모두 담는다', () => {
    world.reset()
    world.spawnItem(variantOf('사과'), 0.3)
    simulate(world, 1)
    const snapshots = world.snapshots()
    expect(snapshots).toHaveLength(1)
    const snapshot = snapshots[0]!
    expect(snapshot.variant.id).toBe('apple')
